...
const ListaReservasPage = () => {
  ...
  const handleCancelarClick = async (reservaId: string) => {
    if (confirm('¿Estás seguro de que deseas cancelar esta reserva?')) {
      try {
        const token = Cookies.get('token');
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reservas/cancelar/${reservaId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="container mx-auto py-8">
        <h1 className="text-4xl font-bold mb-8 text-center">Cancelar Reservas</h1>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {reservas.map((reserva) => (
              <div key={reserva.id} className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition-shadow">
                ...
                <p className="text-gray-600 mb-4">Personas: {reserva.cantidadPersonas}</p>
                <button
